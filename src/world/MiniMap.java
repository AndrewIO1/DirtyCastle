...
public class MiniMap {
	...
	public void updateMiniMap() {
		updateMiniMap(0,0,map.getWidth(),map.getHeight());
		
	}
	
	public void updateMiniMap(int x, int y, int width, int height) {
		int alpha = 255;
		for(int i = x; i < x+width; i++){
			for(int j = y; j < y+height; j++){
				Tile currentTile = map.getTile(i, j, 0);
